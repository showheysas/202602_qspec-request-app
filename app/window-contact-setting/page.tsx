...
export default function WindowContactSettingPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<WindowContact | null>(null);
  ...
  const [formData, setFormData] = useState({
    name: '',
    department: '',
    businessType: 'household' as 'business' | 'household' | 'other',
    category: 'ビールテイスト',
    email: '',
  });
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter((c) => {
      const btMatch = filterBusinessType === 'all' || c.businessType === filterBusinessType;
      const catMatch = filterCategory === 'all' || c.category === filterCategory;
      return btMatch && catMatch;
    });
  }, [contacts, filterBusinessType, filterCategory]);

  // Group by businessType × category
  const groupedContacts = useMemo(() => {
    const groups = new Map<string, WindowContact[]>();
    filteredContacts.forEach((c) => {
      const key = `${BUSINESS_TYPE_LABELS[c.businessType]}×${c.category}`;
  ...
  const openAddForm = () => {
    setFormData({ name: '', department: '', businessType: 'household', category: 'ビールテイスト', email: '' });
    setIsAddMode(true);
    setEditingContact(null);
  };

  const openEditForm = (contact: WindowContact) => {
    setFormData({
      name: contact.name,
  ...
  const handleSave = () => {
    ...
    if (editingContact) {
      ...
      const newContact: WindowContact = {
        id: `wc-new-${Date.now()}`,
        ...formData,
      };
  ...
  return (
    <main className="bg-background py-3 px-4">
      <div className="max-w-full mx-auto">
        <div className="mb-3">
          <h1 className="text-2xl font-bold text-foreground mb-1">窓口担当者設定</h1>
          <p className="text-sm text-muted-foreground">
            事業分類×カテゴリに基づく窓口担当者の登録・修正・削除を行います
          </p>
        </div>
            ...
            <div className="grid grid-cols-5 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, businessType: e.target.value as 'business' | 'household' | 'other' })}
                  ...
                  filteredContacts.map((contact, idx) => (
                    ...
                    >
                      <td className="px-3 py-1.5 text-xs text-foreground">
                        {BUSINESS_TYPE_LABELS[contact.businessType] || contact.businessType}
                      </td>
